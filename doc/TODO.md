07.02.2026
    1) от in+ припаять к входу в dc-dc преобразователь но снизу платы

    +2) заменить транзистор который питает модуль связи
    +3) замерить напряжение к датчикам (бме, часы) - установить 
    +3.1) убедиться что c датчиков считываются поакзания (БМЕ, часы)
    +4) датчик внешний температуры
    +5) датчик внешний уровня воды
    +5) схему выключения протестировать (сигнал DONE)
        нужно от ножки которая идет на лед подать на Плату сигнал ДРВ

    +6) отправлять данные в центр 

    +7) поставить амперметер - будет ли с ним работать - может сгореть к херам..?

    8) подключить все обратно к MQTT + Dashboard + м.б. оставить печать показаний в CSV для отладки
    
    0) доработки прошивки

Отрисовать новый вариант платы чтобы отдать на Завод


Прошивка что доработать:
1) 
...
Не протестировано:
    1) апмерметер - сейчас без него - не понятно вообще... отдельно протестировать, потом попробовать возможно сгорел (скрее всего)
    2) преобразователь дц-дц - сейас без него - можно вернуть предв. протестив или заказав новый    
...
Задачи:
    *) продумать герморазьем
    *) датчики состава воды
    *) nb iot


На 09.12.2025:
    1) работает прошивка с МКТТ - умеет принимать настройки, отправку истории и т.д.
    2) новая сигнализация ошибок через коды

На 07.12.2025
    Доработать:
        1) нужно получать настройки из MQTT

        2) структурировать код

        3) жду плату из Китая чтобы припаять компоненты
    
    Работает:
        1) отправка данных в формате protobuf на MQTT сервер, отображение в Grafana




На 05.12.2025
    1) девайс работает 5 дней все норм.

    2) решено приступить к созданию нормального бекенда


    

На 30.11.2025
    1) в схему внести корретировки что запитываемся от 5В датчик напряжения!

    4) перепроверить еще раз схему

    5) отрисовать в виде печатной платы, утолстить необходимые дорожки (размер 140 на 120 либо 150 на 120)
