...
var messageHandler mqtt.MessageHandler = func(client mqtt.Client, msg mqtt.Message) {
    ...
    //log.Printf("Данные от устройства ID %d: Уровень воды %.2f м, АКБ %.2f В, Т.воды %.2f, %.2f, %.2f, %d",
    //telemetryData.DevId, telemetryData.WaterLevel, telemetryData.UBattery, telemetryData.WaterTemperature, telemetryData.GpsY, telemetryData.GpsX, telemetryData.TimeMount)
    
    // Используем %.6f для координат, чтобы видеть высокую точность
    log.Printf("Данные от устройства ID %d: Уровень воды %.2f м, АКБ %.2f В, Т.воды %.2f, GPS_Y: %.8f, GPS_X: %.8f, TimeMount: %d, %d, %s, %s",
    telemetryData.DevId, telemetryData.WaterLevel, telemetryData.UBattery, telemetryData.WaterTemperature, telemetryData.GpsY, 
                    telemetryData.GpsX, telemetryData.TimeMount, telemetryData.OperatorTime, telemetryData.OperatorName, telemetryData.Imei)
    ...
    correctedTimestampTimeSend := time.Unix(int64(telemetryData.TimeSend), 0).Add(-3 * time.Hour)    
    
    correctedTimestampTimeStart := time.Unix(int64(telemetryData.TimeStart), 0).Add(-3 * time.Hour)    
    
    correctedTimestampOperatorTime := time.Unix(int64(telemetryData.OperatorTime), 0).Add(-3 * time.Hour)    
    
    serverProcessingTimeUTC := time.Now().UTC()
    ...
    p := influxdb2.NewPointWithMeasurement("telemetry").
        ...
        AddField("device_id", fmt.Sprintf("%d", telemetryData.DevId)).
        AddField("firmware_version", telemetryData.Ver).
        AddField("operator_name", telemetryData.OperatorName).
        AddField("operator_time", correctedTimestampOperatorTime).
        AddField("imei", telemetryData.Imei).


        AddField("time_start", correctedTimestampTimeStart).
        AddField("time_send", correctedTimestampTimeSend).
        AddField("time_mount", telemetryData.TimeMount).
        ...
        SetTime(serverProcessingTimeUTC)
